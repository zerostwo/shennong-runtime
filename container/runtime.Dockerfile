...
ARG CRAN_DEBIAN_SIGNING_FPR=95C0FAF38DB3CCAD0C080A7BDC78B2DDEABC47B7
...
RUN apt-get update \
    ...
    && gpg --batch --keyserver hkp://keyserver.ubuntu.com:80 \
       --recv-keys "${CRAN_DEBIAN_SIGNING_FPR}" \
    && test "$(gpg --batch --with-colons --fingerprint "${CRAN_DEBIAN_SIGNING_FPR}" \
       | awk -F: '$1 == "fpr" { print $10; exit }')" = "${CRAN_DEBIAN_SIGNING_FPR}" \
    && gpg --batch --export "${CRAN_DEBIAN_SIGNING_FPR}" \
...
ARG SHENNONG_TARBALL_SHA256=016b1a589d33e46016ad0a8fe4216e03e0860a96f32924af5043d9003cb14377
ARG SHENNONG_DATA_REF=17f0f0e87dd8ad2a3751dd11c58c8aa43823aa69
ARG SHENNONG_DATA_TARBALL_SHA256=97044cbd1dbb5835a712b9594a7cadc5a552c1a784f91a4a2a4d18f5c410350d
...
RUN mkdir -p /opt/shennong/src/shennong /opt/shennong/src/shennong-data \
    ...
    && SHENNONG_REF="${SHENNONG_REF}" SHENNONG_DATA_REF="${SHENNONG_DATA_REF}" \
