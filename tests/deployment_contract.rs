...
fn artifact_reader_uses_networkless_no_follow_openat_and_bounded_copy() {
    ...
    let artifact_reader_config = executor
        .split("let name = format!(\"shennong-artifact-read-{}\"")
        .nth(1)
        .and_then(|source| source.split("let helper_id =").next())
        .expect("artifact reader container config");
    assert!(
        executor.contains(
            "const ARTIFACT_READER_SOURCE: &str = include_str!(\"../container/worker/read_artifact.py\");"
        ),
        "the daemon must embed the current artifact reader source"
    );
    assert!(
